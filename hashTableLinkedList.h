...
public:
    HashTableLL(int sz); //konstruktor
    ~HashTableLL(); //destruktor
...
//konstruktor
template <typename K, typename V>
HashTableLL<K, V>::HashTableLL(int sz) {
    size = sz;
...
//destruktor
template <typename K, typename V>
HashTableLL<K, V>::~HashTableLL() {
    delete[] table;
}

//wybiera odpowiedni dla elementy o kluczu key indeks w tablicy table
template <typename K, typename V>
int HashTableLL<K, V>::hash(K key) {
    return (key % size + size) % size;
}

//wstawienie par w table
template <typename K, typename V>
void HashTableLL<K, V>::insert(K key, V value) {
...
template <typename K, typename V>
bool HashTableLL<K, V>::remove(K key) {
